...
(defun own/sql-connect (product connection)
  ...
  (interactive
   (list
	...
	(completing-read "Connection: " (mapcar '(lambda (elt) (symbol-name (car elt))) sql-connection-alist)))
  (message (concat product " " connection))
  (setq sql-product (make-symbol product))
  (sql-connect (make-symbol connection))))
#+end_src
** Hooks
#+begin_src emacs-lisp :tangle yes
